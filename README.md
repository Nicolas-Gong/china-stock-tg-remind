...
**注意**：添加提醒时，机器人会自动验证股票代码是否存在。如果股票代码无效，系统会提示错误。
...
#### 商品ETF（部分支持）
腾讯财经API对贵金属等商品的支持有限，主要通过ETF形式提供：
- **石油相关**：`OIL` (石油ETF)
- **其他商品**：部分商品ETF可能可用

**注意**：腾讯财经API主要专注于股票和指数，对于现货贵金属、石油等商品的实时价格支持有限。
...
1. **交易时间限制**：机器人只在各市场的交易时间内发送提醒（北京时间）
   - **A股**：周一至周五，9:30-11:30 和 13:00-15:00
   - **港股**：周一至周五，9:30-12:00 和 13:00-16:00
   - **美股**：周一至周五，21:30(今晚)-04:00(次日凌晨)（美东时间9:30-16:00的北京时间对应）
   - **周末**：所有市场都不发送提醒

2. **机器人实例冲突**：Telegram只允许一个机器人实例运行
   ```
...
3. **API限制**：注意股票API的调用频率限制
4. **数据准确性**：确保使用的股票数据API可靠准确
5. **隐私保护**：提醒数据本地存储，不上传到服务器
6. **稳定性**：机器人会自动重试失败的请求
